...
const nombre = ref('')
const municipiosSeleccionados = ref<string[]>([])
...
async function guardar() {
  ...
  enviando.value = true
  const { error: errorCrear } = await crear({ nombre: nombre.value.trim(), municipios: municipiosSeleccionados.value })
...
<template>
  <div class="max-w-2xl">
    ...
    <form class="rounded-lg border border-slate-200 bg-white p-4 mb-6 space-y-3" @submit.prevent="guardar">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Municipios</label>
        <SelectorMunicipios v-model="municipiosSeleccionados" />
      </div>
